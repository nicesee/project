   ...
   $.fn.Flowdesign = function(options)
    ...
    if(processData.list)
    {
        $.each(processData.list, function(i,row)
        {
            ...
            var nodeId = "window" + row.id, badge = 'badge-inverse',icon = 'glyphicon glyphicon-random';
            ...
            .attr("process_name",row.process_name)
            .attr("process_to",row.process_to)
            .attr("process_id",row.id)
            .addClass("process-step btn btn-primary")
            .html('<span class="process-flag badge '+badge+'"><i class="'+icon+'"></i></span>&nbsp;' + row.process_name )
        ...
        getProcessInfo:function()
        {
            ...
            $("#leipi_process_info input[type=hidden]").each(function(i){
                ...
                if(processVal.length==2)
                {
                    ...
                    {
                        aProcessData[processVal[0]] = {"id":processVal[0], "top":0,"left":0,"process_to":[]};
                    }
          ...
          _canvas.find("div.process-step").each(function(i){ //生成Json字符串，发送到服务器解析
                  if($(this).attr('id')){
                      ...
                      var process_tos = aProcessData[id];
                      if(process_tos) {
                          for(var i=0;i<process_tos.process_to.length;i++) {
                              if(process_to != "") {
                                  process_to += "," + process_tos.process_to[i];
                              }else {
                                  process_to = process_tos.process_to[i];
                              }
                          }
                      ...
                      list.push({"id":id,"process_name":process_name,"process_to":process_to,"style":style})
                  }
              });
